...
\youranswer{Question 3 - Discuss whether varying width affects the results in a consistent way, and whether the results are expected and match well with the prior knowledge (by which we mean your expectations as are formed from the relevant Theory and literature)
    ...
    We can see that whilst increasing the width improves the accuracy of the model over the given training data, the accuracy for the validation data decrease as the width increases, this is characterised by the generalisation error of the model, which decreases with the width. The reason for this is due to the model overfitting to the data, in the context of the generalisation error this corresponds to the variance term of the generalisation error bound.
}
...
\youranswer{Question 5 - Discuss whether varying depth affects the results consistently, and whether the results are expected and match well with the prior knowledge (by which we mean your expectations as are formed from the relevant Theory and literature)

}
    The same issue for width occurs as well for depth, except that it seems the validation error changes more for each change in layer, this makes sense given the fact that each layer introduces 128 more neurons, as opposed to the width change, therefore the overfitting should as a result be worse simply due to their being more parameters.
...
Both the L$1$ and L$2$ penalty can be combined to regularise the objective function together like so 
$$
\mathcal{L}(\theta)
=
\mathcal{L}_{\text{Cross Entropy}}(\theta;\mathcal{D})
+
\lambda_{1}
\|\theta\|_{1}
+
...
\begin{figure}[t]
    ...
    \begin{subfigure}{\linewidth}
        \includegraphics[width=\linewidth]{figures/dim_accuracy.pdf}
        \caption{accuracy by epoch}
        \label{fig:width_acccurves}
    \end{subfigure} 
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/dim_error.pdf}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}{.475\linewidth}
        \includegraphics[width=\linewidth]{figures/dropout_vs_accuracy_and_gap.pdf}
        \caption{Accuracy and error by inclusion probability.}
        \label{fig:dropoutrates}
    \end{subfigure} 
    \begin{subfigure}{.475\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/weight_decay_vs_accuracy_and_gap.pdf}
...
\begin{figure*}[t]
    \centering
    \begin{subfigure}{\linewidth}
        \includegraphics[width=\linewidth]{figures/L1L2genacc.pdf}
        \label{fig:customtrainingaccuracy}
    \end{subfigure} 
    \caption{Training Accuracy, Error, and Gradient Flow Across Layers for the 4 experiment runs for the permutations of the joint L1 and L2 penalty terms.}
    \label{fig:hp_search}
\end{figure*}
}
}
...
\begin{table*}[t]
    ...
    \begin{tabular}{c|c|ccc}
    \toprule
        Model & Hyperparameter values $(\lambda_1, \lambda_2)$ & Validation accuracy (\%) & Train Error & Validation Error \\
    \midrule
    \midrule
        \multirow{4}*{$L_{1}$–$L_{2}$ Mix Penalty}
                 & (5e-4,5e-4) & 79.4 & 0.623 & 0.642 \\
                 & (5e-4,1e-3)        & 78.7 & 0.660 & 0.677 \\
                 & (1e-4,1e-3)               & 84.4 & 0.441 & 0.475 \\
                 & (1e-5,1e-3)               & 85.0 & 0.379 & 0.438 \\
    \bottomrule
    \end{tabular}
    \caption{Final validation accuracy and error values for the $L_{1}$–$L_{2}$ mixed regularisation experiments. Each model was trained with the same architecture and initialization but different $(\lambda_1, \lambda_2)$ coefficients for the respective $L_{1}$ and $L_{2}$ penalties.}
    \label{tab:l12_mix_results}
\end{table*}
