...
class EMNISTDataProvider(DataProvider):
    ...
    def label_smoothing(self, int_targets, alpha=0.1):
        ...
        """
        # Actually I dont even need the input do I...
        one_hot = self.to_one_of_k(int_targets)
        # ones_smoothing = lambda x: 1 - alpha
        # zeros_smoothing = lambda x: (alpha)/(self.num_classes - 1)
        
        result = one_hot.copy()
        result[one_hot == 1] = 1 - alpha #ones_smoothing(int_targets[int_targets == 1])
        result[one_hot == 0] = (alpha)/(self.num_classes - 1) # zeros_smoothing(int_targets[int_targets == 0])
        return result
